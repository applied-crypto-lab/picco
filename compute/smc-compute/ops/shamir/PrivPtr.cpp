...
int is_repeated_listnode(dlist list, listnode node, int level, int type, NodeNetwork net, int id, SecretShare *ss) {
    ...
    while (tmp != list->tail) {
        if (level == 0) {
            if ((type == 0 && tmp->u.int_var_location == node->u.int_var_location) || (type == 1 && tmp->u.float_var_location == node->u.float_var_location) || (type == 2 && tmp->u.struct_var_location == node->u.struct_var_location)) {
