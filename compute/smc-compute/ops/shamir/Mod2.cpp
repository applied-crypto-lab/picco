...
// Protocol 3.4 page 7
void doOperation_Mod2(mpz_t *result, mpz_t *input, int K, int size, int threadID, NodeNetwork net, SecretShare *ss) {
    ...
    mpz_init(Bit[0]);
    PRandM(1, size, R, threadID, net, ss);
    PRandInt(K, 1, size, S, threadID, ss);
    ss->modAdd(C, input, R[0], size);
