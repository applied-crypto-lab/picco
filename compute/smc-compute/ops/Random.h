...
public:
	Random(NodeNetwork nodeNet, std::map<long, std::vector<int> > poly, int nodeID, SecretShare *s);
	virtual ~Random();
	...
	static void getNextRandValue(int id, int bits, std::map<long, std::vector<int> > poly, mpz_t value);  
	void getNextRandValue(int id, int bits, std::map<long, std::vector<int> > poly, mpz_t value, int threadID);  
private:
	int numOfThreads;
