...
void Random::getNextRandValue(int id, int bits, std::map<long, std::vector<int> > poly, mpz_t value)
{
	if(isFirst == 0)
	{
		...
		for(int i = 0; i < poly.size(); i++)
		{
			...
			gmp_randseed_ui(rstates[i], keys[i]); 
		}
		isFirst = 1; 
	}	
	else
...
void Random::getNextRandValue(int id, int bits, std::map<long, std::vector<int> > poly, mpz_t value, int threadID)
{
	if(threadID == -1){
	...
	if(isFirst_thread[threadID] == 0)
	{
		...
		for(int i = 0; i < poly.size(); i++)
		{
			...
			gmp_randseed_ui(rstates_thread[i][threadID], keys[i]); 
		}
...
Random::Random(NodeNetwork nodeNet, std::map<long, std::vector<int> > poly, int nodeID, SecretShare *s) {
    ss = s;
    id = nodeID;
    net = nodeNet;
    polynomials = poly;
...
void Random::PRandBit(int size, mpz_t* results){
	...
	generateRandValue(id, ss->getBits(), size, u);
...
void Random::PRandBit(int size, mpz_t* results, int threadID){
	...
	/***********************************************************/
	generateRandValue(id, ss->getBits(), size, u, threadID);
	ss->modMul(v, u, u, size);
