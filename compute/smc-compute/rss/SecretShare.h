...
template <typename T>
void SecretShare<T>::prg_aes(uint8_t *dest, uint8_t *src, __m128i *ri) {
    __m128i rr, mr;
    __m128i *r = ri;
...
template <typename T>
void SecretShare<T>::prg_setup_3() {
...
template <typename T>
void SecretShare<T>::prg_setup_5() {
    // need to create numShares+1 keys, random containers, etc
    uint numKeys = numShares + 1;
    random_container = new uint8_t *[numKeys];
    for (int i = 0; i < numKeys; i++) {
...
template <typename T>
void SecretShare<T>::prg_setup_mp_7() {
    // need to create numShares+1 keys, random containers, etc
    uint numKeys = numShares + 1;
    random_container = new uint8_t *[numKeys];
    for (int i = 0; i < numKeys; i++) {
...
template <typename T>
void SecretShare<T>::prg_getrandom(int keyID, uint size, uint length, uint8_t *dest) {
...
template <typename T>
void SecretShare<T>::prg_getrandom(uint size, uint length, uint8_t *dest) {
...
template <typename T>
uint SecretShare<T>::getNumShares() {
    return numShares;
}

template <typename T>
uint SecretShare<T>::getTotalNumShares() {
    return totalNumShares;
}

template <typename T>
uint SecretShare<T>::nCk(uint n, uint k) {
