 ...
 */
static int copyprivate_arguments_from_varlist(astdecl d,
    ...
    if (symtab_get(stab, d->u.id, IDNAME)->isarray || /* already ptrs */
        ...
        sa = UnaryOperator(UOP_addr, Identifier(d->u.id));
        ra = CommaList(ast_expr_copy(sa),
...
static int copyprivate_arguments(ompclause t, astexpr *sendargs, astexpr *recvargs) {
    int nvars = 0;
 ...
 */
static void copyprivate_statements(ompcon t, aststmt *send, aststmt *recv) {
...
void xform_single(aststmt *t) {
    ...
    *t = BlockList(
        ...
        BlockList(
            If(
                FunctionCall(Identifier(Symbol("ort_mysingle")),
