...
static aststmt linepragma(int line, symbol file) {
...
void xform_atomic(aststmt *t) {
    ...
    if (ex->type == ASS &&
        ...
        tmp = Declaration(
            (ex->left->type != IDENT ? Declspec(SPEC_long, 0) : ast_spec_copy_nosc(symtab_get(stab, ex->left->u.sym, IDNAME)->spec)),
            InitDecl(
 ...
 */
static void declare_private_dataclause_vars(ompclause t) {
    if (t == NULL)
        return;
    if (t->type == OCLIST) {
 ...
 */
static void xform_ompcon_body(ompcon t) {
...
void ast_omp_xform(aststmt *t) {
    ...
    case DCPARFOR: {
        ...
        enum dircontype type = ((*t)->u.omp->type == DCPARFOR ? DCFOR_P : DCSECTIONS); /* Special node */
 ...
 */
static void xform_thread_functions(funclist l) {
    if (l != NULL) {
...
static void place_thread_functions(funclist l) {
    aststmt neu, bl;
