 ...
 * contain a usertype.
 */
static astdecl multi2single_field(astspec spec, astdecl decl) {
    if (decl)
...
static void xt_break_multifield(astdecl *t) {
    if (*t == NULL)
        return;
 ...
 */
static void xt_suedecl_xform(aststmt *d) {
    aststmt n;
    astspec s;
    int su; /* 1 if struct or union, 0 if enum */

    assert((*d)->type == DECLARATION);
    if ((s = speclist_getspec((*d)->u.declaration.spec, SUE, 0)) == NULL)
        return;
    ...
    n->is_stmt_for_sng = (*d)->is_stmt_for_sng;
 ...
 */
static astdecl xt_substitute_decl(astdecl origdecl, astdecl subst) {
    astdecl copy, id;
    ...
    if (!(subst->spec == NULL && /* if a DIDENT/DPAREN but not a pointer .. */
          subst->decl != NULL && (subst->decl->type == DIDENT || subst->decl->type == DPAREN || subst->decl->type == DFUNC))) {
...
astdecl xt_concrete_to_abstract_declarator(astdecl orig) {
    ...
    if (orig == NULL)
        return NULL;
    if (c->type == ABSDECLARATOR)
        return (ast_decl_copy(orig));
    ...
    if (c != NULL)
        ...
        case DPAREN:
            assert(c->decl != NULL);
            a = (c->decl->type == DIDENT) ? NULL : xt_concrete_to_abstract_declarator(c->decl);
            break;
        case DARRAY:
            a = ArrayDecl(xt_concrete_to_abstract_declarator(c->decl),
                          (c->spec != NULL && c->spec->type == SPEC && c->spec->subtype == SPEC_star) ? ast_spec_copy(c->spec) : NULL,
                          ast_expr_copy(c->u.expr));
            break;
        case DFUNC:
            a = FuncDecl(xt_concrete_to_abstract_declarator(c->decl),
                         (c->u.params && c->u.params->type == DLIST && c->u.params->subtype == DECL_idlist) ? NULL : ast_decl_copy(c->u.params));
            break;
        default:
...
void xt_barebones_substitute(astspec *spec, astdecl *decl) {
    ...
    {
        if (subst) {                                             /* Was abstract declarator obvioulsy */
...
static void xt_retire(aststmt t) {
    if (retiretree == NULL)
        retiretree = t;
    else
...
static aststmt multi2single_stmt(astspec spec, astdecl decl) {
    if (decl->type == DLIST && decl->subtype == DECL_decllist) {
 ...
 */
static void xt_break_multidecl(aststmt *t) {
    aststmt n =
...
int xt_symbol_depends_on_sue(symbol s) {
    ...
    return (e && (xt_spec_depends_on_sue(e->spec) || xt_decl_depends_on_sue(e->decl)));
}
...
void xt_decl_array2pointer(astdecl d) {
    ...
    case DPARAM:
        if (d->decl) {
            if ((d->spec->subtype != SPEC_int && d->spec->subtype != SPEC_float) && (d->spec->body == NULL || d->spec->body->subtype == SPEC_public))
                xt_decl_array2pointer(d->decl);
        }
        break;
