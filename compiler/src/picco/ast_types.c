...
static int _two_longs(astspec s, int one_long) {
    if (s == NULL)
        return (0);
    if (s->type == SPECLIST) {
...
nt_size speclist_size(astspec s) {
    if (speclist_has_longlong(s))
        return (LONGLONG);
    if (speclist_getspec(s, SPEC, SPEC_long) != NULL)
        return (LONG);
    if (speclist_getspec(s, SPEC, SPEC_short) != NULL)
        return (SHORT);
    return (NORMAL);
}

nt_sign speclist_sign(astspec s) {
    if (speclist_getspec(s, SPEC, SPEC_unsigned) != NULL)
        return (UNSIGNED);
    return (SIGNED);
}
...
nt_basetype speclist_basetype(astspec s) {
    ...
    if (speclist_getspec(s, SPEC, SPEC_char) != NULL)
        return (CHAR);
    if (speclist_getspec(s, SPEC, SPEC_float) != NULL)
        return (FLOAT);
    if (speclist_getspec(s, SPEC, SPEC_double) != NULL)
        return (DOUBLE);
    if (speclist_getspec(s, SPEC, SPEC_ubool) != NULL)
        return (UBOOL);
    return (INT);
}
