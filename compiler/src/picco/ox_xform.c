...
static void oxcon_xform(oxcon *t) {
    if ((*t)->body)
        ast_stmt_xform(&((*t)->body));
}

static aststmt oxdir_commented(oxdir d) {
    static str bf = NULL;
    aststmt st;
...
static taskfunc get_taskfunc(symbol fid) {
    taskfunc tf;
            ...
            for (j = 0; j < tf->np; j++)
                if (tf->p[j].pid == p->size_expr->u.sym) {
                    if (decl_ispointer(tf->p[j].decl))
                        t = FunctionCall(
                            Identifier(Symbol("torc_set_outsize")),
                            CommaList(Identifier(p->pid), Identifier(tf->p[j].pid)));
                    else
                        t = FunctionCall(
                            Identifier(Symbol("torc_set_outsize")),
                            CommaList(Identifier(p->pid),
                                      UnaryOperator(UOP_addr, Identifier(tf->p[j].pid))));
                    break;
                };
 ...
 */
static aststmt Callbackfunction(oxcon c, taskfunc tf) {
    aststmt cbf;
    astdecl fid;
    int i;
    tfparam *p;
...
static void ox_xform_taskdef(aststmt *t) {
...
static void ox_xform_taskschedule(aststmt *t) {
    ...
    if (ox_has_clause((*t)->u.ox->directive->clauses, OX_OCUNTIED))
        tied = 0;
    else if (ox_has_clause((*t)->u.ox->directive->clauses, OX_OCTIED))
        tied = 1;
    ...
    if (scope)
        scopetype = scope->u.value;

    f = Expression(
        FunctionCall(
            Identifier(Symbol("torc_taskschedule")),
            CommaList(
                CommaList(
                    CommaList(
                        numConstant(scopetype == OX_SCOPE_NODES ? 0 : scopetype == OX_SCOPE_WLOCAL ? 1
                                                                  : scopetype == OX_SCOPE_WGLOBAL  ? 2
                                                                                                   : -1), /* 1 */
                        (start) ? ast_expr_copy(start->u.expr) : numConstant(-1)                         /* 2 */
                        ),
                    (stride) ? ast_expr_copy(stride->u.expr) : Constant(strdup("(1<<31)-1")) /* 3 */
                    ),
...
static void ox_xform_tasksync(aststmt *t) {
...
static void checkNstore_varlist_vars(astdecl d, int clausetype, int operator) {
    stentry e;
    symbol vid;
...
static void checkNstore_dcclause_vars(oxclause t) {
    if (t->type == OX_OCLIST) {
...
static int count_paramlist_vars(astdecl d) {
    int n = 0;
...
static tfparam *info_paramlist_vars(astdecl d, tfparam *p) {
    int type;
...
static tfparam *types_paramlist_vars(astdecl d, tfparam *p) {
    int type;
        ...
        if (!d->spec) /* int by default */
            ...
            if (speclist_getspec(d->spec, SUE, 0) ||
                ...
                exit_error(1, "(%s, line %d) OMPi-X error:\n\t"
                           ...
                           d->file->name, d->l);
            type = speclist_getspec(d->spec, SPEC, SPEC_char) ? OX_TYPE_CHAR : speclist_getspec(d->spec, SPEC, SPEC_float) ? OX_TYPE_FLOAT
                                                                           : speclist_getspec(d->spec, SPEC, SPEC_double)  ? OX_TYPE_DOUBLE
                                                                                                                           : OX_TYPE_INT;
            type |=
                (speclist_getspec(d->spec, SPEC, SPEC_short) ? OX_SIZER_SHORT : speclist_getspec(d->spec, SPEC, SPEC_long) ? OX_SIZER_LONG
                                                                                                                           : OX_SIZER_DEFAULT);
            type |=
                (speclist_getspec(d->spec, SPEC, SPEC_unsigned) ? OX_GAMUT_UNSIGNED : OX_GAMUT_SIGNED);
        }
        p->type = type;
...
void tf_sizeexpr_make(astexpr *tree) {
    ...
    case IDENT: {
        ...
        if (p != NULL) /* yeap */
        {
            ...
            ast_expr_free(*tree);
            *tree = UnaryOperator(UOP_paren, ast_expr_copy(_callargs[i]));
        }
        break;
    }
    case DOTDES:
...
static char *inparam(int i) {
    sprintf(inparam_str, "_ompix_%d", i);
    return (inparam_str);
}
#define CALLWAY(p) ((p).intent == OX_OCREDUCE ? CALL_BY_RED((p).op) : (p).intent == OX_OCOUT ? CALL_BY_RES \
 ...
 */
static astexpr callargs_tree(astexpr *ca, taskfunc tf) {
    astexpr expr, e;
    int i;
    ...
    str_truncate(typstr);
    str_printf(typstr, "%d", tf->p[0].type);
    ...
    for (i = 1; i < tf->np; i++) /* .. and the rest */
    {
        ...
        str_truncate(typstr);
        str_printf(typstr, "%d", tf->p[i].type);
    ...
    for (i = 0; i < tf->np; i++) /* finally the actual arguments */
        expr = CommaList(expr, (tf->p[i].size == 1 && tf->p[i].intent == OX_OCIN) ? UnaryOperator(UOP_addr, Identifier(Symbol(inparam(i)))) : ast_expr_copy(ca[i]));

    return (expr);
}

static astexpr *tabulate_callargs(astexpr e, astexpr *ca) {
    if (e == NULL)
        return (NULL);
    if (e->type == COMMALIST)
...
static void free_callargs_table(int n, astexpr *ca) {
    for (--n; n >= 0; n--)
        ast_expr_free(ca[n]);
    free(ca);
}

static int count_callargs(astexpr e) {
    if (e == NULL)
        return (0);
    else if (e->type != COMMALIST)
        return (1);
    else
        return (count_callargs(e->left) + count_callargs(e->right));
}

static aststmt ox_func2task_call(astexpr *fc, taskfunc tf, int detached, int tied,
                                 int atall, astexpr atnode, astexpr atworker) {
    ...
    *fc = FunctionCall(
        Identifier(Symbol("torc_create_ox")),
        CommaList(
            CommaList(
                CommaList(
                    atall ? numConstant(-2) : /* 1 (atnode(all)) */
                        atnode ? ast_expr_copy(atnode)
                               :                         /* 1 (atnode(expr)) */
                ...
                CommaList(
                    ...
                    numConstant(tied == -1 ? -1 : tied == 0 ? 0
                                                            : 1) /* 4 (tied flag) */
                    )),
            CommaList(
                CommaList(
                    Identifier(tf->tfid),                                    /* 5 (function) */
                    tf->has_callback ? Identifier(callback_func(tf->tfid)) : /* 6 (callback) */
                        CastedExpr(                                          /* 6 (NULL callback) */
                                   Casttypename(
                                       Declspec(SPEC_void, 0),
                                       AbstractDeclarator(Pointer(), NULL)),
                                   numConstant(0))),
                tf->np == 0 ? numConstant(0) : /* 7 (0 args) */
                    CommaList(
...
static astexpr ox_atnode(oxdir d) {
    oxclause e;

    return ((e = ox_has_clause(d->clauses, OX_OCATNODE)) ? e->u.expr : NULL);
}

static astexpr ox_atworker(oxdir d) {
    oxclause e;
...
void ox_xform_task(aststmt *t) {
    ...
    tempdecls = ox_func2task_call(&((*t)->u.ox->body->u.expr), tf,
                                  ox_has_clause((*t)->u.ox->directive->clauses, OX_OCDETACHED) ? 1 : 0,
                                  ox_has_clause((*t)->u.ox->directive->clauses, OX_OCTIED) ? 1 : ox_has_clause((*t)->u.ox->directive->clauses, OX_OCUNTIED) ? 0
                                                                                                                                                            : -1,
                                  ox_has_clause((*t)->u.ox->directive->clauses, OX_OCATALL) ? 1 : 0,
                                  ox_atnode((*t)->u.ox->directive), ox_atworker((*t)->u.ox->directive));
    new = BlockList(oxdir_commented((*t)->u.ox->directive),
                    (tempdecls) ? Compound(BlockList(tempdecls, (*t)->u.ox->body)) : (*t)->u.ox->body);
