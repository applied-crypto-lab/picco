...
static void new_thrfunc() {
    thrnum++;
    sprintf(_tfn, "_thrFunc%d_", thrnum);
}

static void new_parreduction() {
    par_red_num++;
...
static void store_varlist_vars(astdecl d, int clausetype, int opid) {
    stentry e;
...
static void store_dataclause_vars(ompclause t) {
    if (t->type == OCLIST) {
...
static void xp_store_dataclause_vars(ompdir d) {
...
static astspec xp_thrarg_struct() {
    stentry e, orig;
    astdecl st = NULL, tmp;
...
astexpr xp_parstruct_initializer() {
    ...
    for (e = sng_vars->top; e; e = e->stacknext) {
        tmp = (threadmode && symtab_get(stab, e->key, IDNAME)->isthrpriv) ? Identifier(e->key) : /* 'cause it is already a pointer!!! */
...
void xp_sharedng_add_fpredgvars() {
    ...
    for (e = dataclause_vars->top; e; e = e->stacknext)
        if ((isFirstprivate(e->key) || isReduction(e->key) || symtab_get(stab, e->key, IDNAME)->isthrpriv) && isGlobal(stab, e->key))
 ...
 */
static void xp_fiparray_initialization(symbol var, astexpr init) {
 ...
 */
static aststmt xp_copyin_copy_stmt(symbol var) {
    stentry e = symtab_get(stab, var, IDNAME);
    astexpr init;

    init = /* ( e->scopelevel == -10 ) ?
             Identifier( tp_new_name(var) ) : */
           /* The initializer/source */
        UnaryOperator(
            UOP_star,
    ...
    if (!e->isarray)      /* Plain assignment */
        return Expression(/* *var = <orig_var> */
                          Assignment(
...
static void xp_copyin_initialization(symbol var) {
    ...
    if (op == OC_min || op == OC_max)
        ...
        st = Expression(
            Assignment(
                ...
                (op != OC_land && op != OC_lor) ? Identifier(d->u.id) : BinaryOperator((op == OC_land) ? BOP_land : BOP_lor, UnaryOperator(UOP_star, UnaryOperator(UOP_paren, PtrField(Identifier(Symbol(SHVARSNAME)), d->u.id))), Identifier(d->u.id))));
 ...
 */
static aststmt xp_copyin_declarations() {
    stentry e, orig;
    aststmt st = NULL, tmp;

    for (e = dataclause_vars->top; e; e = e->stacknext) {
        if (!isCopyin(e->key) || !threadmode)
            continue;
        orig = symtab_get(stab, e->key, IDNAME);
...
astexpr xp_numthreads(ompdir d) {
    return (d->clauses ? xp_ifnumthreads_clause(d->clauses, OCNUMTHREADS) : NULL);
}
                                          ...
                                          CommaList(
                                              ...
                                              CastedExpr(
                                                  Casttypename(
                                                      Declspec(SPEC_void, 0),
                                                      AbstractDeclarator(Pointer(), NULL)),
                                                  has_sng_vars() ? UnaryOperator(UOP_addr, Identifier(Symbol(SHVARSNAME))) : numConstant(0))),
