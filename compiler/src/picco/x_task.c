...
static void store_varlist_vars(astdecl d, int clausetype) {
    stentry e;
...
static void xt_store_dataclause_vars(ompdir d) {
...
static astspec xt_taskenv_struct() {
    stentry e, orig;
    astdecl st = NULL, tmp;
 ...
 */
static aststmt xt_array_initialize_stmt(symbol var) {
    needMemcpy = 1;
    return Expression(
        FunctionCall(
 ...
 */
static void xt_fpsnapshot_initialization(symbol var, int isarray) {
    aststmt st;
 ...
 */
static aststmt dupcode_private_declarations(aststmt *fpinits) {
    stentry e;
    ...
    for (e = sng_vars->top; e; e = e->stacknext) {
        ...
        tmp = Expression(
            Assignment(
                PtrField(Identifier(Symbol(SHVARSNAME)), e->key),
                ASS_eq,
                (threadmode && orig->isthrpriv) ? Identifier(e->key) : /* 'cause it is already a pointer */
...
static aststmt xt_sue_declarations() {
    stentry e;
    aststmt all = NULL, d;
...
void xform_task(aststmt *t) {
    ...
    *t = Compound(
                           ...
                           CommaList(
                               ...
                               CastedExpr(
                                   Casttypename(
                                       Declspec(SPEC_void, 0),
                                       AbstractDeclarator(Pointer(), NULL)),
                                   (has_tsng_vars() || has_tfp_vars()) ? Identifier(Symbol(SHVARSNAME)) : numConstant(0)))))));
    ...
    if (enableCodeDup && mergeable) {
        *t = If((ifexpr ? BinaryOperator(BOP_lor,
