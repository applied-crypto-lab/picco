...
/* Non-global */
static aststmt tp_ng_new(symbol var) {
...
/* For global ones */
static aststmt tp_g(symbol var) {
    stentry e = symtab_get(stab, var, IDNAME);
    aststmt new;
...
static aststmt tp_procmode(symbol var) {
    symtab_get(stab, var, IDNAME)->isthrpriv = 1;
    return (NULL);
}

static aststmt thrpriv_handlelist(astdecl d) {
            ...
            FunctionCall(
                ...
                CommaList(
                    CommaList(
                        ...
                        Sizeof(baseisptr ? UnaryOperator(UOP_star, UnaryOperator(UOP_paren, base)) : base)),
                    /* **MUST** copy the "base", not use it for a 2nd time! */
                    baseisptr ? ast_expr_copy(base) : UnaryOperator(UOP_addr, ast_expr_copy(base))))));
}
