 ...
 * that matter.
 */
static void analyze_omp_for(aststmt s,
    ...
    if (init->type == EXPRESSION) /* assignment: var = lb */
    {
        ...
        *var = e->left->u.sym;
        *lb = e->right;
    } else if (init->type == DECLARATION) /* declaration: type var = lb */
    {
    ...
    } else /* ASS */
    {
        ...
        if (incr->opid != ASS_eq) {
            ...
            *incrop = tmp->opid;
            ...
            {
                if (tmp->left->type != IDENT || tmp->left->u.sym != *var) { /* var = incr + var */
...
void xform_for(aststmt *t) {
    ...
    do {
        ...
        if ((varentry = symtab_get(dvars, var, IDNAME)) == NULL) {
            ...
            else if (varentry->ival == OCFIRSTLASTPRIVATE)
        ...
        if (chsize == NULL) {
            ...
            if (schedchunk || schedtype == OC_runtime) /* expr for chunk size */
            {
                stmp = BlockList(
                    stmp,
                    Declaration(
                        Speclist_right(Declspec(SPEC_public, 0), Declspec(SPEC_int, 0)), (schedtype == OC_runtime) ? Declarator(NULL, IdentifierDecl(Symbol(chsize))) : InitDecl(Declarator(NULL, IdentifierDecl(Symbol(chsize))), ast_expr_copy(schedchunk))));
            }
        }
        ...
        expr = CommaList(
            CommaList(
                ...
                CastedExpr(
                    Casttypename(Speclist_right(Declspec(SPEC_public, 0), Declspec(SPEC_long, 0)), NULL),
                    e3)),
            (collapsenum == 1) ? CastedExpr(/* (void *) 0 */
    ...
    if (schedtype == OC_affinity) {
        ...
        e7->flag = PUB;
        s = For(Expression((collapsenum > 1) ? Assignment(Identifier(Symbol("iter_")),
                                                          ASS_eq,
                ...
                ((collapsenum > 1) ? e7 : CommaList(e7, Assignment(Identifier(var), ASS_add, ast_expr_copy(step)))),
