...
void ast_declare_decllist_vars(astspec s, astdecl d) {
    ...
    if (d->type == DINIT)
        d = (ini = d)->decl; /* Get rid of the initializer */
    ...
    if (d->decl != NULL && d->decl->type != ABSDECLARATOR) {
        ...
        e = symtab_put(stab, t, speclist_getspec(s, STCLASSSPEC, SPEC_typedef) ? TYPENAME : (kind == DFUNC) ? FUNCNAME
                                                                                                            : IDNAME);
        e->spec = s;
        e->decl = d;
        e->idecl = ini;
...
static void ast_declare_paramlist_vars(astdecl d) {
...
static void ast_omp_dataclause_vars(ompclause t) {
    if (t == NULL)
        return;
...
static void ast_ompdir_vars(ompdir t) {
    if (t->clauses) {
...
static void ast_ompix_nondataclause_vars(oxclause t) {
    if (t->type == OX_OCLIST) {
        if (t->u.list.next != NULL)
...
static void ast_oxdir_vars(oxdir t) {
    if (t->clauses) {
...
static struct {
    symtab clausevars; /* all vars from the clauses */
...
void mark_var(symbol v) {
    ...
    if ((type = is_interesting_var(v)) == 0)
        return;
    e = symtab_get(stab, v, IDNAME);
    if (e->scopelevel > thislevel)
        return; /* Don't care for those */
...
int is_interesting_var(symbol v) {
    ...
    {
        if ((f = symtab_get(taskinfo.clausevars, v, IDNAME)) != NULL) /* explicit */
            return (f->ival == OCSHARED ? (interesting & VARS_TSNG) : f->ival == OCFIRSTPRIVATE ? (interesting & VARS_TFP)
                                                                                                : 0);
        if (taskinfo.defnone)
            exit_error(1,
                       "openmp error:\n\t"
...
static void find_global_vars(aststmt t, int xformflag) {
    if (checkGTP) {
        if (gtp_vars == NULL)
            gtp_vars = Symtab();
        else
 ...
 */
static void find_paracon_sng_vars(ompcon t, int xformflag) {
    if (sng_vars == NULL)
        sng_vars = Symtab();
    else
