...
void ast_batch_allocate_counter() {
    ...
    while (tmp != NULL) {
        if (tmp->flag == 1) {
            indent();
            fprintf(output, "int* _picco_batch_array%d = (int*)malloc(sizeof(int) * _picco_batch_counter%d);\n", size, tmp->batch_index);
            indent();
...
void ast_batch_allocate_array_for_private_indexed_element(astexpr tree, int batch_index, int private_index) {
    if (tree->ftype == 0) {
        indent();
        fprintf(output, "int* _picco_private_indexed_inputarray%d = (int*)malloc(sizeof(int) * _picco_batch_counter%d);\n", private_index, batch_index);
        indent();
        fprintf(output, "int* _picco_private_indexed_outputarray%d = (int*)malloc(sizeof(int) * _picco_batch_counter%d);\n", private_index, batch_index);
        indent();
        fprintf(output, "for (int _picco_i = 0; _picco_i < _picco_batch_counter%d; _picco_i++)\n", batch_index);
        indent();
        fprintf(output, "{\n");
        ...
        fprintf(output, "ss_init(_picco_private_indexed_inputarray%d[_picco_i]);\n", private_index);
        indent();
        fprintf(output, "_picco_private_indexed_outputarray%d[_picco_i] = (int*)malloc(sizeof(int) * 4);\n", private_index);
        indent();
...
void ast_batch_allocate_array_for_narrayelement(astexpr tree, int batch_index, int narray_element_index) {
    if (tree->flag == PUB) {
        ...
        if (tree->ftype == 0) {
            ...
            indlev++;
            indent();
            fprintf(output, "_picco_batch_tmp_array%d[_picco_i] = (int*)malloc(sizeof(int) * 4);\n", narray_element_index);
            indent();
            fprintf(output, "for (int _picco_j = 0; _picco_j < 4; _picco_j++)\n");
            indlev++;
            indent();
...
void ast_compute_condition(condnode cnode, branchnode bnode, int thread_id) {
    ...
    indent();
    fprintf(output, "int _picco_condtmp%d;\n", height);
    indent();
...
void ast_expr_pmalloc_show(astexpr tree) {
    ...
    case SPEC:
        if (tree->u.dtype->spec->subtype == SPEC_int) {
            ...
            indent();
            indent();
            fprintf(output, "_picco_temp_[_picco_i] = (int*)malloc(sizeof(int) * 4);\n");
            indent();
            indent();
            fprintf(output, "for (int _picco_j = 0; _picco_j < 4; _picco_j++)\n");
            indlev++;
    ...
    case DECLARATOR:
        ...
        if (spec->subtype == SPEC_float || spec->subtype == SPEC_Rlist && (spec->body->subtype == SPEC_private && spec->u.next->subtype == SPEC_float)) {
            ...
            if (tree->spec) {
                ...
                if (!flag) {
                    fprintf(output, "%s%s = (int*)malloc(sizeof(int) * (4));\n", struct_name, tree->decl->u.id->name);
                    indent();
                    fprintf(output, "for (int _picco_i = 0; _picco_i < 4; _picco_i++)\n");
                    indlev++;
                    indent();
...
void ast_print_struct_helper_function(astspec tree) {
    ...
    if (!struct_node_get_flag(sns, tree->name->name)) {
        while (field != NULL) {
            ...
            if (!ispointer) {
                if (spec_type == 0)
                    sprintf(field_type_signature, "%s", "int");
                else if (spec_type == 1)
                    sprintf(field_type_signature, "%s", "int*");
            } else {
            ...
            fprintf(output, "void %s(priv_ptr %s, %s _picco_val, %sint _picco_tag, int _picco_priv_cond, int _picco_thread_id)\n", field_func_signature, struct_ptr, field_type_signature, array_index);
...
void ast_print_priv_field(astdecl tree, astspec spec) {
    ...
    case DECLARATOR:
        ...
        if (spec->subtype == SPEC_int) {
            fprintf(output, "int %s", tree->decl->u.id->name);
            break;
        }
        if (spec->subtype == SPEC_float) {
            fprintf(output, "int* %s", tree->decl->u.id->name);
        ...
        if (spec->subtype == SPEC_int) {
            if (tree->decl->type == DIDENT)
                ...
                if (tree->decl->decl->type == DIDENT)
                    fprintf(output, "int** %s", tree->decl->decl->u.id->name);
            }
    ...
    case DECLARATOR:
        ...
        if (spec->subtype == SPEC_float || spec->subtype == SPEC_Rlist && spec->u.next->subtype == SPEC_float) {
            ...
            if (tree->spec) {
                ...
                fprintf(output, "int* %s; \n", tree->decl->u.id->name);
                indent();
                fprintf(output, "%s = (int*)malloc(sizeof(int) * (4));\n", tree->decl->u.id->name);
                indent();
                fprintf(output, "for (int _picco_i = 0; _picco_i < 4; _picco_i++)\n");
                indlev++;
                indent();
...
void ast_decl_memory_assign_int(astdecl tree, char *prefix) {
    indent();
    if (tree->decl->type == DIDENT) {
        fprintf(output, "%s%s = (int*)malloc(sizeof(int) * (", prefix, tree->decl->u.id->name);
        ast_expr_show(tree->u.expr);
    } else if (tree->decl->type == DARRAY) {
        fprintf(output, "%s%s = (int**)malloc(sizeof(int*) * (", prefix, tree->decl->decl->u.id->name);
    ...
    if (tree->decl->type == DIDENT) {
        ...
        indlev++;
        indent();
        fprintf(output, "%s%s[_picco_i] = (int*)malloc(sizeof(int) * (%s));\n", prefix, tree->decl->decl->u.id->name, str_string(arg_str));
        indent();
...
void ast_decl_memory_assign_float(astdecl tree, char *prefix) {
    ...
    if (tree->decl->type == DIDENT) {
        ...
        indlev++;
        indent();
        fprintf(output, "%s%s[_picco_i] = (int*)malloc(sizeof(int) * (4));\n", prefix, tree->decl->u.id->name);
        indent();
        fprintf(output, "for (int _picco_j = 0; _picco_j < 4; _picco_j++)\n");
        indent();
        indent();
        ...
        indlev++;
        indent();
        fprintf(output, "%s%s[_picco_i][_picco_j] = (int*)malloc(sizeof(int) * (4));\n", prefix, tree->decl->decl->u.id->name);
        indent();
        fprintf(output, "for (int _picco_k = 0; _picco_k < 4; _picco_k++)\n");
        indent();
        indent();
...
void ast_float_tmp_decl_show(char *prefix, int sidx, int eidx) {
    if (eidx >= sidx) {
        for (ind = sidx; ind <= eidx; ind++) {
            indent();
            fprintf(output, "int* %stmp%d = (int*)malloc(sizeof(int) * 4);\n", prefix, ind);
            indent();
            fprintf(output, "for(int _picco_j = 0; _picco_j < 4; _picco_j++)\n");
            indlev++;
            indent();
...
void ast_temporary_variable_declaration() {
    ...
    if (is_priv_int_index_appear || is_priv_float_index_appear) {
        indent();
        fprintf(output, "int _picco_priv_ind1, _picco_priv_ind2, _picco_priv_ind3;\n");
        indent();
    ...
    if (is_priv_int_index_appear || is_priv_int_ptr_appear) {
        indent();
        fprintf(output, "int _picco_priv_tmp1, _picco_priv_tmp2;\n");
        indent();
    ...
    if (is_priv_float_index_appear || is_priv_float_ptr_appear) {
        indent();
        fprintf(output, "int *_picco_priv_ftmp1 = (int*)malloc(sizeof(int) * 4);\n");
        indent();
        fprintf(output, "int *_picco_priv_ftmp2 = (int*)malloc(sizeof(int) * 4);\n");
        indent();
        fprintf(output, "for(int i = 0; i < 4; i++){\n");
        indlev++;
        indent();
    ...
    if (is_priv_int_struct_field_appear) {
        indent();
        fprintf(output, "int _picco_str_field_tmp_int1, _picco_str_field_tmp_int2, _picco_str_field_tmp_int3;\n");
        indent();
    ...
    if (is_priv_float_struct_field_appear) {
        ...
        indent();
        fprintf(output, "int *_picco_str_field_tmp_float1 = (int*)malloc(sizeof(int) * 4);\n");
        indent();
        fprintf(output, "int *_picco_str_field_tmp_float2 = (int*)malloc(sizeof(int) * 4);\n");
        indent();
        fprintf(output, "int *_picco_str_field_tmp_float3 = (int*)malloc(sizeof(int) * 4);\n");
        indent();
        fprintf(output, "for(int i = 0; i < 4; i++){\n");
        indlev++;
        indent();
