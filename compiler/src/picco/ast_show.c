...
case FUNCDEF:
      ...
      if((tree->u.declaration.spec->subtype == SPEC_Rlist && tree->u.declaration.spec->u.next->subtype == SPEC_void)
	 || tree->u.declaration.spec->subtype == SPEC_void)
		is_return_void = 1; 
...
void ast_print_struct_helper_function(astspec tree)
{
	...
	while(field != NULL)
		...
		if(level > 0)
		{
        	...
        	indent();fprintf(output, "__s->smc_dereference_read_ptr(_picco_tmp_ptr, _picco_val, 1, _picco_priv_cond, \"%s\", _picco_thread_id);\n", temp_type);
		indlev--;  
		indent();fprintf(output, "}\n"); 
		indent();fprintf(output, "else\n"); 
		indlev++; 
        	indent();fprintf(output, "__s->smc_dereference_write_ptr(_picco_tmp_ptr, _picco_val, 1, _picco_priv_cond, \"%s\", _picco_thread_id);\n", temp_type);
		indlev--; 
...
void ast_float_tmp_decl_show(char* prefix, int sidx, int eidx){
    if(eidx >= sidx){
        for(ind = sidx; ind <= eidx; ind++)
        {
            indent();fprintf(output, "mpz_t* %stmp%d = (mpz_t*)malloc(sizeof(mpz_t) * 4);\n", prefix, ind);
            indent();fprintf(output, "for(int _picco_j = 0; _picco_j < 4; _picco_j++)\n");
            indlev++;
            indent();
...
void ast_float_tmp_clear_show(char* prefix, int sidx, int eidx)
{
	if(eidx > 0){
	    for(ind = sidx; ind <= eidx; ind++){
            	...
            	indlev--;
		indent();fprintf(output, "free(%s%d);\n", prefix, ind); 
		}
	}
}
