    ...
    case SRETURN:
        ...
        // assume for now, the return value is always an identifier
            ...
            if (technique_var == REPLICATED_SS)
                fprintf(output, "ss_set(rvar, %s, __s->getNumShares())", tree->u.expr->u.sym->name);
            else
...
static aststmt decomp_tmp_batch[100]; /* BATCH tree that owns this temp (for computing temp size from loop bounds) */
...
/* Emit a C expression for the 1D decomp temp size for a specific batch.
 * Temp is indexed by the loop variable, so size must bound max(loop_var)+1.
 * Looks at init (start value) and cond (bound + comparison op) to compute:
 *   i < N      → N
 *   i <= N     → N+1
 *   i > N / i >= N (reverse) → start+1
 * Falls back to the batch iteration counter if the shape is unrecognized. */
static void emit_decomp_temp_size(aststmt batch_tree, int fallback_counter_idx) {
    if (!batch_tree || batch_tree->type != BATCH) {
        fprintf(output, "_picco_batch_counter%d", fallback_counter_idx);
        return;
    }
    aststmt init = batch_tree->u.iteration.init;
    astexpr cond = batch_tree->u.iteration.cond;
    astexpr init_rhs = NULL;
    if (init && init->type == EXPRESSION && init->u.expr &&
        init->u.expr->type == ASS)
        init_rhs = init->u.expr->right;

    if (cond && cond->type == BOP && cond->right) {
        astexpr bound = cond->right;
        int op = cond->opid;
        if (op == BOP_lt) {
            fprintf(output, "(");
            ast_expr_show(bound);
            ...
            fprintf(output, ") + 1)");
            return;
        } else if ((op == BOP_gt || op == BOP_geq) && init_rhs) {
            fprintf(output, "((");
            ast_expr_show(init_rhs);
            fprintf(output, ") + 1)");
            return;
        }
    }
...
static void decompose_bop_recursive(aststmt *stmt_list, astexpr expr, astexpr loop_var, aststmt batch_tree) {
    if (!expr) return;
    /* Unwrap parentheses: recurse into UOP_paren, then propagate replacement up */
    if (expr->type == UOP && expr->opid == UOP_paren) {
        decompose_bop_recursive(stmt_list, expr->left, loop_var, batch_tree);
    ...
    if (expr->right && (expr->right->type == BOP ||
        ...
        decompose_bop_recursive(stmt_list, expr->right, loop_var, batch_tree);
    }
            ...
            if (sel_loop_var && tree->body && tree->body->type == EXPRESSION &&
                ...
                if ((bop_count > 1 || has_nested) && rhs->type == BOP) {
                    ...
                    if (rhs->left) {
                        ...
                        if (dl && dl->type == BOP)
                            decompose_bop_recursive(&decomposed, rhs->left, sel_loop_var, batch_tree);
                    }
                    if (rhs->right) {
                        astexpr dr = unwrap_paren(rhs->right);
                        if (dr && dr->type == BOP)
                            decompose_bop_recursive(&decomposed, rhs->right, sel_loop_var, batch_tree);
                    }
                    if (decomposed) {
        ...
        if (tree->u.expr && tree->u.expr->type == ASS) {
            ...
            if ((bop_count > 1 || has_nested_bop) && rhs->type == BOP) {
                ...
                if (rhs->left) {
                    astexpr dl = unwrap_paren(rhs->left);
                    if (dl && dl->type == BOP)
                        decompose_bop_recursive(&decomposed, rhs->left, loop_var, batch_tree);
                }
                if (rhs->right) {
                    astexpr dr = unwrap_paren(rhs->right);
                    if (dr && dr->type == BOP)
                        decompose_bop_recursive(&decomposed, rhs->right, loop_var, batch_tree);
                }
...
void ast_stmt_batch_show(aststmt tree, branchnode current) {
    ...
    for (int _dt = 1; _dt <= decomp_total; _dt++) {
        ...
        if (decomp_tmp_ftype[_dt] == 0 && !decomp_tmp_is_2d[_dt]) {
            /* 1D int temp array — sized to bound the loop variable, not iteration count.
             * Needed for loops with step!=1, non-zero start, or reverse direction. */
            fprintf(output, "int _decomp_sz%d = ", _dt);
            emit_decomp_temp_size(decomp_tmp_batch[_dt], max_batch_index);
            fprintf(output, ";\n");
            indent();
            fprintf(output, "priv_int* _picco_decomp_tmp%d = (priv_int*)malloc(sizeof(priv_int) * _decomp_sz%d);\n", _dt, _dt);
            indent();
            fprintf(output, "for (int _picco_i = 0; _picco_i < _decomp_sz%d; _picco_i++)\n", _dt);
            indent();
            fprintf(output, "{\n");
            indlev++;
            indent();
    ...
    for (int _dt = 1; _dt <= decomp_total; _dt++) {
        if (decomp_tmp_ftype[_dt] == 0 && !decomp_tmp_is_2d[_dt]) {
            /* 1D int cleanup */
            indent();
            fprintf(output, "for (int _picco_j = 0; _picco_j < _decomp_sz%d; _picco_j++) {\n", _dt);
            indlev++;
            indent();
    ...
    if (tree->left->type == IDENT) {
        ...
        */
            ...
            // the second dimension is privately indexed
                ...
                indent();
                if (technique_var == REPLICATED_SS) {
                    fprintf(output, "__s->smc_set((int)(");
                    ast_expr_show(e2);
                    fprintf(output, "), %s, -1, -1, \"int\", %d);\n", name, tree->thread_id);
                } else {
...
// ast_mult_operation_batch_expr_show
    ...
    while (tmp != NULL) {
        if (tmp->flag == 1) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                fprintf(output, "ss_init(_picco_batch_array%d[_picco_i], __s->getNumShares());\n", size);
            indlev--;
            indent();
            fprintf(output, "}\n");
            size--;
...
void ast_batch_allocate_array_for_private_indexed_element(astexpr tree, int batch_index, int private_index) {
    if (tree->ftype == 0) {
        ...
        if (technique_var == SHAMIR_SS)
            ...
            fprintf(output, "ss_init(_picco_private_indexed_inputarray%d[_picco_i], __s->getNumShares());\n", private_index);
        indent();
        if (technique_var == SHAMIR_SS)
            fprintf(output, "ss_init(_picco_private_indexed_outputarray%d[_picco_i]);\n", private_index);
        else if (technique_var == REPLICATED_SS)
            fprintf(output, "ss_init(_picco_private_indexed_outputarray%d[_picco_i], __s->getNumShares());\n", private_index);
        indlev--;
        indent();
        fprintf(output, "}\n");
        ...
        if (technique_var == SHAMIR_SS)
            ...
            fprintf(output, "ss_init(_picco_private_indexed_outputarray%d[_picco_i][_picco_j], __s->getNumShares());\n", private_index);
        indlev--;
        indent();
        fprintf(output, "}\n");
        indlev--;
    ...
    if (tree->flag == PUB) {
        ...
        if (tree->ftype == 0) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                fprintf(output, "ss_init(_picco_batch_tmp_array%d[_picco_i], __s->getNumShares());\n", narray_element_index);
            indlev--;
            indent();
            fprintf(output, "}\n");
        } else if (tree->ftype == 1) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                fprintf(output, "ss_init(_picco_batch_tmp_array%d[_picco_i][_picco_j], __s->getNumShares());\n", narray_element_index);
            indlev--;
            indent();
            fprintf(output, "}\n");
            indlev--;
...
void ast_print_smc_high_dim_show(aststmt tree) {
    // Both RSS and Shamir: loop over first dimension, call 1D overload for each row
    ...
    fprintf(output, ", \"%s\", %d);\n", SPEC_symbols[tree->u.smcops.type->subtype], tree->u.smcops.variable->thread_id);
    indlev--;
    indent();
    fprintf(output, "}");
}
...
void ast_compute_condition(condnode cnode, branchnode bnode, int thread_id) {
    ...
    indent();
    if (technique_var == REPLICATED_SS)
        fprintf(output, "ss_init(_picco_condtmp%d, __s->getNumShares());\n", height);
    else
    ...
    if (bnode->key == 0) {
        if (technique_var == REPLICATED_SS)
            fprintf(output, "ss_set(_picco_condtmp%d, _picco_tmp%d, __s->getNumShares());\n", height, index);
        else
...
void ast_init_temp_for_struct_field(char **var, int ispointer, int field_type) {
    if (ispointer) {
        ...
        if (field_type == 0) {
            indent();
            if (technique_var == REPLICATED_SS)
                fprintf(output, "__s->smc_set(0, %s, -1, -1, \"int\", -1);\n", *var);
            else
            ...
            indent();
            if (technique_var == REPLICATED_SS)
                fprintf(output, "__s->smc_set(0, %s[_picco_ind], -1, -1, \"int\", -1);\n", *var);
            else
...
// This is the function that takes care of the lists mentioned in pointers paper for keeping the ptr array indexes
 ...
 **/
// The new ast_decl_show, ast_exp_show and ast_show was created to store the loops from batch statements. This is currently used only for batch statements and for regular cases it uses the same stream as the rest of the program 
...
// Print specification of tree representing a type or a declaration (This version was added while adding support for multiple batch operations in one line and for optimizing batch code)
// The new ast_decl_show, ast_exp_show and ast_spec_show was created to store the loops from batch statements. This is currently used only for batch statements and for regular cases it uses the same stream as the rest of the program 
        ...
        if (spec->subtype == SPEC_int || (spec->subtype == SPEC_Rlist && (spec->body->subtype == SPEC_private && spec->u.next->subtype == SPEC_int))) {
            ...
            if (tree->spec) {
                ...
                if (technique_var == SHAMIR_SS)
                    ...
                    fprintf(output, "ss_init(%s%s, __s->getNumShares());\n", struct_name, tree->decl->u.id->name);
            } else {
                fprintf(output, "\n\n");
                if (technique_var == SHAMIR_SS)
...
void ast_decl_stmt_show(aststmt tree, branchnode current) {
    ...
    if (tree->u.declaration.spec->subtype == SPEC_int || tree->u.declaration.spec->subtype == SPEC_float) {
        ...
        // Added support for array init in one line -> this was not added before
            ...
            // private int
                ...
                if (is_priv == 1 && gflag == 1 && is_init_decl == 1) {
                    if (technique_var == SHAMIR_SS) {
                        ...
                        str_printf(global_string, "ss_init(%s, __s->getNumShares());\n", tree->decl->u.id->name);
                    }
                } else {
                    if (technique_var == SHAMIR_SS) {
                        fprintf(output, "ss_init(%s);\n", tree->decl->u.id->name);
                    } else if (technique_var == REPLICATED_SS){
                        fprintf(output, "ss_init(%s, __s->getNumShares());\n", tree->decl->u.id->name);
                    }
                }
            }
            break;
            ...
            if (tree->spec) {
                ...
                if (is_priv == 1 && gflag == 1 && is_init_decl == 1){
                    if (technique_var == SHAMIR_SS) {
                        ...
                        indent_global_string(global_string);
                        indent_global_string(global_string);
                        str_printf(global_string, "ss_init(%s[_picco_i], __s->getNumShares());\n", tree->decl->u.id->name);
                    }
                    indlev--;
                    indent_global_string(global_string);
                    str_printf(global_string, "}\n");
                    ...
                    if (technique_var == SHAMIR_SS) {
                        ...
                        indent();
                        indent();
                        fprintf(output, "ss_init(%s[_picco_i], __s->getNumShares());\n", tree->decl->u.id->name);
                    }
                    indlev--;
                    indent();
                    fprintf(output, "}\n");
...
void ast_decl_memory_assign_int(astdecl tree, char *prefix) {
    ...
    if (is_priv == 1 && gf == 1 && is_init_decl == 1) {
        indent_global_string(global_string);
        str_printf(global_string, "for (int _picco_i = 0; _picco_i < ");  // this is where the struct elements gets printed and the issue I had with init for struct that was getting printed to where the global variables should be declared, I fixed this by adding extra layer of checking flags
    } else 
        fprintf(output, "for (int _picco_i = 0; _picco_i < ");
    if (tree->decl->type == DIDENT) {
        ast_expr_show(tree->u.expr);
    ...
    if (tree->decl->type == DIDENT) {
        ...
        if (is_priv == 1 && gf == 1 && is_init_decl == 1) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                str_printf(global_string, "ss_init(%s%s[_picco_i], __s->getNumShares());\n", prefix, tree->decl->u.id->name);
            }
        } else {
            if (technique_var == SHAMIR_SS)
                fprintf(output, "ss_init(%s%s[_picco_i]);\n", prefix, tree->decl->u.id->name);
        ...
        if (is_priv == 1 && gf == 1 && is_init_decl == 1) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                str_printf(global_string, "ss_init(%s%s[_picco_i][_picco_j], __s->getNumShares());\n", prefix, tree->decl->decl->u.id->name);
        } else {
            if (technique_var == SHAMIR_SS)
                fprintf(output, "ss_init(%s%s[_picco_i][_picco_j]);\n", prefix, tree->decl->decl->u.id->name);
            else if (technique_var == REPLICATED_SS)
                fprintf(output, "ss_init(%s%s[_picco_i][_picco_j], __s->getNumShares());\n", prefix, tree->decl->decl->u.id->name);
        }
        indlev--;
        indent();
        if (is_priv == 1 && gf == 1 && is_init_decl == 1) {
...
void ast_decl_memory_assign_float(astdecl tree, char *prefix) {
    ...
    if (tree->decl->type == DIDENT) {
        ...
        if (technique_var == SHAMIR_SS) {
            ...
            // RSS 2D float: FE[M][N] → FE[M][N][4][numShares]
            // Level 1: N columns (same as Shamir)
            if (gf == 1) {
                indent_global_string(global_string);
                str_printf(global_string, "%s%s[_picco_i] = (priv_int**)malloc(sizeof(priv_int*) * (%s));\n", prefix, tree->decl->decl->u.id->name, str_string(arg_str));
            } else
        ...
        if (technique_var == SHAMIR_SS) {
            ...
            // RSS 2D float: loop over N columns
            if (gf == 1) {
                indent_global_string(global_string);
                str_printf(global_string, "for (int _picco_j = 0; _picco_j < %s; _picco_j++) {\n", str_string(arg_str));
            } else
        ...
        if (technique_var == SHAMIR_SS) {
            ...
            // RSS 2D float: Level 2 = 4 float components (same as Shamir)
            if (gf == 1) {
                indent_global_string(global_string);
                str_printf(global_string, "%s%s[_picco_i][_picco_j] = (priv_int*)malloc(sizeof(priv_int) * (4));\n", prefix, tree->decl->decl->u.id->name);
            } else {
        ...
        if (technique_var == SHAMIR_SS) {
            ...
            // RSS 2D float: loop over 4 float components
            if (gf == 1) {
                indent_global_string(global_string);
                str_printf(global_string, "for (int _picco_k = 0; _picco_k < 4; _picco_k++) {\n");
            } else {
        ...
        if ( technique_var == SHAMIR_SS) {
            ...
            // RSS 2D float: Level 3 = numShares per component
            if (gf == 1) {
                indent_global_string(global_string);
                str_printf(global_string, "ss_init(%s%s[_picco_i][_picco_j][_picco_k], __s->getNumShares());\n", prefix, tree->decl->decl->u.id->name);
            } else {
                fprintf(output, "ss_init(%s%s[_picco_i][_picco_j][_picco_k], __s->getNumShares());\n", prefix, tree->decl->decl->u.id->name);
            }
        }
        indlev--;
        indent();
...
void ast_tmp_decl_show(char *prefix, int sidx, int eidx) {
    if (eidx >= sidx) {
        for (ind = sidx; ind <= eidx; ind++) {
            indent();
            fprintf(output, "priv_int %stmp%d;\n", prefix, ind);
        }
        for (ind = sidx; ind <= eidx; ind++) {
            indent();
            if (technique_var == SHAMIR_SS)
                ...
                fprintf(output, "ss_init(%stmp%d, __s->getNumShares());\n", prefix, ind);
        }
        fprintf(output, "\n");
    }
}
    ...
    if (eidx >= sidx) {
        for (ind = sidx; ind <= eidx; ind++) {
            ...
            if (technique_var == SHAMIR_SS)
                ...
                fprintf(output, "ss_init(%stmp%d[_picco_j], __s->getNumShares());\n", prefix, ind);
            indlev--;
            indent();
            fprintf(output, "}\n");
        }
                ...
                if (tree->left->arraytype == 0 && tree->right->ftype != tree->left->ftype) {
                    ...
                    if (technique_var == SHAMIR_SS) {
                        if(immresulttype == 0) { // print the rest of the instruction for smc_set - old/regular
                            ...
                            // Generate Shamir-style calls for RSS to match function signatures
                            if (tree->left->arraysize == NULL) {
                                // Scalar: use Shamir-style signature (no array size parameter)
                                if (tree->right->ftype == 1) { // float
                                    if (is_init_decl == 1)
                                        str_printf(global_string, "(%s, %s, %d, %d, %d, %d, \"float\", %d)", str_string(rightop), str_string(leftop), tree->right->size, tree->right->sizeexp, tree->left->size, tree->left->sizeexp, tree->right->thread_id);
                                    else
                                ...
                                // Array: include array size parameter
                                str array_Size = Str("");
                                if (tree->left->arraysize && tree->left->arraysize->type == IDENT)
                                    str_printf(array_Size, tree->left->arraysize->u.sym->name);
                                else if (tree->left->arraysize && tree->left->arraysize->type == CONSTVAL)
                                ...
                                // Fallback for expression-type array sizes (e.g., computed sizes)
                                if (strcmp(str_string(array_Size), "") == 0 && tree->left && tree->left->arraysize) {
                                    ast_expr_print(array_Size, tree->left->arraysize);
                                }
...
void ast_array_tmp_clear_show(astexpr tree, int dim, int array_int_tmp_index) {
    ...
    if (dim == 1) { // one-dimensional
        if (technique_var == SHAMIR_SS) {
            ...
            for (int i = 1; i <= array_int_tmp_index; i++) {
                fprintf(output, "for (int _picco_i = 0; _picco_i < ");
                ast_expr_show(tree->left->arraysize);  // Use array size like Shamir
                fprintf(output, "; _picco_i++) {\n");
                fprintf(output, "   ss_clear(_picco_arr_tmp%d[_picco_i]);\n", i);
                fprintf(output, "}\n");
                fprintf(output, "free(_picco_arr_tmp%d);\n\n", i);
            ...
            if (dim == 1) {
                if (technique_var == SHAMIR_SS) {
                    ...
                    for (int i = 1; i <= array_int_tmp_index; i++) {
                        ...
                        ast_expr_show(tree->left->arraysize);  // Print the array size
                        fprintf(output, "; _picco_i++) {\n");
                        fprintf(output, "   ss_init(_picco_arr_tmp%d[_picco_i], __s->getNumShares());\n", i);
                        fprintf(output, "}\n\n");
                    }
                }
            } // Else if dim == 2
            ...
            if (dim == 1) {
                if (technique_var == SHAMIR_SS) {
                    for (int i = 1; i <= array_float_tmp_index; i++) {
                        ...
                        ast_expr_show(tree->left->arraysize);  // Print the array size
                        fprintf(output, "; _picco_i++) {\n");
                        fprintf(output, "      _picco_arr_ftmp%d[_picco_i] = (priv_int*)malloc(sizeof(priv_int) * (4));\n", i);
                        fprintf(output, "      for (int _picco_j = 0; _picco_j < 4; _picco_j++) {\n");
                    ...
                    for (int i = 1; i <= array_float_tmp_index; i++) {
                        ...
                        fprintf(output, "      _picco_arr_ftmp%d[_picco_i] = (priv_int*)malloc(sizeof(priv_int) * (4));\n", i);
                        fprintf(output, "      for (int _picco_j = 0; _picco_j < 4; _picco_j++) {\n");
                        fprintf(output, "            ss_init(_picco_arr_ftmp%d[_picco_i][_picco_j], __s->getNumShares());\n", i);
                        fprintf(output, "      }\n");
                        fprintf(output, "}\n\n");
                    }
                }
...
void ast_temporary_variable_declaration() {
    ...
    indent();
    fprintf(output, "void* _picco_temp_ __attribute__((unused));\n ");
    if (is_priv_int_index_appear || is_priv_float_index_appear) {
        print_int("_picco_priv_ind1");
        print_int("_picco_priv_ind2");
        print_int("_picco_priv_ind3");
    }

    if (is_priv_int_index_appear || is_priv_int_ptr_appear) {
        print_int("_picco_priv_tmp1");
        print_int("_picco_priv_tmp2");
    }
    if (is_priv_float_index_appear || is_priv_float_ptr_appear) {
        print_float("_picco_priv_ftmp1");
        print_float("_picco_priv_ftmp2");
    }
    if (is_priv_int_ptr_appear) {
        indent();
    ...
    if (is_priv_int_struct_field_appear) {
        print_int("_picco_str_field_tmp_int1");
        print_int("_picco_str_field_tmp_int2");
        print_int("_picco_str_field_tmp_int3");
    }
    ...
    if (is_priv_float_struct_field_appear) {
        print_float("_picco_str_field_tmp_float1");
        print_float("_picco_str_field_tmp_float2");
        print_float("_picco_str_field_tmp_float3");
    }
